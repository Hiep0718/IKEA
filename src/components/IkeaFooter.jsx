...
// Remove direct Ant Design imports and replace with simpler components
...
const IkeaFooter = () => {
  const footerSections = [
    ...
    },
    {
      ...
      items: [
        ...
        { text: "Explore the New IKEA App", isLink: true }
      ]
    },
    {
      title: "About IKEA",
  ...
  const paymentMethods = [
    "visa", "mastercard", "discover", "amex", "ikea-card", 
    "paypal", "apple-pay", "google-pay", "klarna", 
    "unionpay", "jcb", "diners", "gift-card", "affirm"
  ];
              ...
              {section.items.map((item, itemIndex) => (
                <li key={itemIndex}>
                  {item.buttonText ? (
                    ...
                    <a 
                      href="#" 
                      className={`text-sm hover:underline ${item.highlight ? 'font-medium' : ''}`}
                    >
                      {item.text}
                    </a>
                  ) : (
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex flex-wrap gap-2 ml-auto">
            {paymentMethods.map((method, index) => (
              <div key={index} className="w-10 h-6 bg-white border border-gray-200 rounded flex items-center justify-center">
                <img 
                  src={`/src/assets/payment-icons/${method}.png`} 
